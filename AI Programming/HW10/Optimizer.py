...
class SimulatedAnnealing(MetaHeuristics):
    ...
    def run(self,p):
        ...
        value = p.evaluate(current)  # 할당받은 current를 이용해 초기값 value를 구한다.
        ...
        while i < self._limitStuck:
            ...
            next_current = p.randomMutant(current)
            next_value = p.evaluate(next_current)
            ...
            if next_value < value:                
                ...
                if random.uniform(0, 1) < float(math.exp((next_value - value) / temp)): # 0.0 ~ 1.0의 확률보다
                    current = next_current         # (math.exp((next_value - value) / temp)) 이 크게 되면 ( 즉, 확률을 만족할 때 )
                    value = next_value
                    i += 1
          
            if temp < 0.3:   # 0에 근접하다고 생각 되는 0.3이라는 숫자를 설정하여 temp가 0.3보다 작으면 탈출한다.
                p.setSolution(current) # 생성된 객체 p를 통해 p.setSolution 함수를 호출한다.
                p.setMinimum(value) # 생성된 객체 p를 통해 p.setMinimum 함수를 호출한다.
                break
                
            
        self._whenBest = p.getNumEval()                 
        p.setSolution(current) # 생성된 객체 p를 통해 p.setSolution 함수를 호출한다.
        p.setMinimum(value) # 생성된 객체 p를 통해 p.setMinimum 함수를 호출한다.
